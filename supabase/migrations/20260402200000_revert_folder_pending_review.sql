-- Revert folder-source duplicates to auto-skip (no pending_review gate).
-- DuplicateReview component is now a recovery tool, not a mandatory gate.
-- All sources now get acquisition_status = 'duplicate' when a fingerprint
...
BEGIN
    ...
    IF NEW.status = 'done' THEN
        ...
        WHEN 'fingerprint' THEN
            ...
            -- Duplicate detected → stop pipeline (all sources treated the same)
